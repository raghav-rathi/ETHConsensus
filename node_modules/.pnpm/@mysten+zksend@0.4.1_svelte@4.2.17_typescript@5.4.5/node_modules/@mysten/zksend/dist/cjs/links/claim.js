...
loadBag_fn = async function() {
  ...
  const itemIds = bagField.data?.content?.fields?.value?.fields?.item_ids.fields.contents;
  this.creatorAddress = bagField.data?.content?.fields?.value?.fields?.owner;
  if (!itemIds) {
...
executeSponsoredTransactionBlock_fn = async function(input, keypair) {
  return __privateMethod(this, _fetch, fetch_fn).call(this, `transaction-blocks/sponsor/${input.digest}`, {
    method: "POST",
    body: JSON.stringify({
      signature: (await keypair.signTransactionBlock((0, import_utils.fromB64)(input.bytes))).signature
    })
  });
};
_fetch = new WeakSet();
